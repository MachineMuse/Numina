...
import net.minecraft.network.packet.{Packet131MapData, Packet250CustomPayload}
...
abstract class MusePacket(val player: Player) {
  ...
  def getPacket250: Packet250CustomPayload = {
    ...
    write()
    new Packet250CustomPayload(MusePacketHandler.networkChannelName, bytes.toByteArray)
  }

  def getPacket131: Packet131MapData = {
    val packetID = MusePacketHandler.packagers.inverse.get(packager).get.toShort
    write()
    PacketDispatcher.getTinyPacket(Numina, packetID, bytes.toByteArray)
  }
